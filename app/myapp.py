...
def listChannelVideo():
    channelids = ['UCFs9CScYko4OFUzpBepZ88g'] #'UCne2IBkAj3JoyzNAOzXxKMg'
...
def spreadsheettest():
    ...
    channelid = 'UCFs9CScYko4OFUzpBepZ88g' #'UCne2IBkAj3JoyzNAOzXxKMg'
...
def cmdchannelvideo():
        ...
        'videos':[
            {
                ...
                'detail':{
                    'snippet':{
                        'title':'JiNSの株主優待券を使ってオンラインショップでメガネを注文！しかしスクラッチを削りすぎてコードが読めなくなる事態に！'
                    }
                }
            }
        ]
    ...
    db = get_db()
    db.execute(
                    "INSERT INTO channel_video (channelid,video) VALUES(?,json(?))",
                    (channel.get('channelid'),
                    json.dumps(channel.get('videos'))),
                ) 
    db.commit()  """
...
@app.cli.command("testcheck")
@with_appcontext
def cmdtestcheck():
    youtubechecker.send_notify_from_user('118135263309656121264','EwbnopzzsmnP3wc1Ua0of7WKZAGss0uSsnnvql1nFjN')    
